            ...
            <div class="multiselect-field flex flex-col">
                ...
                >
                    <template slot="maxElements">
                        {{
                            __("novaMultiselect.maxElements", {
                    ...
                    <template slot="noOptions">
                        {{
                            field.apiUrl
                                ? __("novaMultiSelect.startTypingForOptions")
                    ...
                    <template slot="clear">
                        <div
                            ...
                            v-if="
                                field.nullable &&
                                (isMultiselect ? value.length : value)
                            "
...
export default {
    ...
    mounted() {
        ...
        if (this.field.dependsOn) {
            ...
            Nova.$on(
                `multiselect-${this.safeDependsOnAttribute}-input`,
                (values) => {
                    ...
                    values.forEach((option) => {
                        ...
                        Object.keys(
                            this.field.dependsOnOptions[option.value] || {}
                        ).forEach((value) => {
                            // Only add unique
                            if (newOptions.find((o) => o.value === value))
                                return;

                            let label = this.field.dependsOnOptions[
                                option.value
                            ][value];
                    ...
                    const hasValue = (value) =>
                        this.options.find((v) => v.value === value);
                    if (this.isMultiselect) {
                        if (Array.isArray(this.value)) {
                            this.value = this.value.filter(
                                (v) => !!v && !!hasValue(v.value)
                            );
                        }
                    } else {
                        this.value =
                            this.value && !!hasValue(this.value.value)
                                ? this.value
                                : void 0;
                    }
                    ...
                    if (dependsOnMax) {
                        const maxValues = values.map((option) => {
                            return (
                                option &&
                                (this.field.dependsOnMax[option.value] || null)
                            );
                        });
    ...
    methods: {
        ...
        fetchOptions: debounce(async function (search) {
            const { data } = await Nova.request().get(`${this.field.apiUrl}`, {
                params: { search },
            });
            ...
            this.asyncOptions = Object.entries(data).map((entry) => ({
                label: entry[1],
                value: entry[0],
            }));
            this.isLoading = false;
