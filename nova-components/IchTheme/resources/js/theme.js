...
function overrideStyles() {
    ...
    const css =
        "html, button, input, optgroup, select, textarea, .chartist-tooltip, .font-sans, .font-serif {" +
        "font-family: " +
        window.config.nova_rtl_theme.font_family +
        ";" +
        "}";
        ...
        const observer = new MutationObserver(() => {
            ...
            popovers.forEach((popover) => {
                setTimeout(() => {
                    ...
                    const translate3d = extractTransformValues(
                        popover.style.transform
                    );
                    const translateX = distanceFromLeft(
                        popover,
                        placement,
                        popOverParent(popover.id)
                    );

                    styleSheet.insertRule(
                        `#${popover.id} {transform: translate3d(${translateX}px, ${translate3d[1]}, ${translate3d[2]}) !important;}`
                    );
                }, 0);
            });
        });
